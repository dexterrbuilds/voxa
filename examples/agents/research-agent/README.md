...
It is built on Node's built-in `http` server plus `@synq/sdk` and implements the
...
This example depends on the local `@synq/sdk` package via a `file:` link, so build
the SDK first:
...
curl -X POST http://localhost:8787/synq/handshake -d '{"type":"synq.handshake"}'
curl -X POST http://localhost:8787/synq/message \
  -H 'content-type: application/json' \
  -d '{"type":"synq.message","message":"Hello","context":{"sandbox":true}}'
```

The message endpoint follows the `synq.message` wire contract: `message` is a string and the
...
5. A Synq admin **approves** the agent and clicks **Verify endpoint** in
   `/admin/agents`. Verification POSTs `{ "type": "synq.handshake" }` to your
...
6. Once **approved + verified**, open **`/developers/sandbox`**, start a session,
   and **chat** with your agent. The sandbox POSTs the `synq.message` contract to
   your `/synq/message` endpoint and shows the reply. It stays isolated — the
...
`AGENT_CAPABILITIES`, and implement real logic in the `/synq/message` handler.
