...
- **Path A — ACTIVE (current MVP), call-to-wake, turn-based:**
  ...
  **ephemeral** LiveKit participant (`agent:nova:playback:<uuid>`). Each turn opens and
  ...
  - **Short-term session memory (per room):** each user prompt + Nova reply is persisted
    ...
    deleted by the SECURITY DEFINER function `cleanup_nova_room_memory(room_id)`, called
  ...
  - **Capture length is silence-based, not a fixed window.** A Web Audio
    ...
    after about 2s of post-speech silence, with about a 15s hard-cap fallback. Tunables
    live in `app/lib/voice-activation.ts` (`NEXT_PUBLIC_NOVA_SILENCE_TIMEOUT_MS`,
    `NEXT_PUBLIC_NOVA_MAX_RECORDING_MS`, `NEXT_PUBLIC_NOVA_SILENCE_THRESHOLD`).
    `NEXT_PUBLIC_NOVA_LISTEN_WINDOW_MS` is legacy/unused for capture length. Nova's UI
    states are **In Room** (idle; wake worker may be armed locally, NOT recording) →
  ...
  - **Two ways to start a Path-A capture:** (1) **call-to-wake** — the default, a
    ...
    **Gated on Nova being in the room:** the worker and the mic permission it needs only
    start when `connected && novaInRoom`; if no active/default agent is invited, tapping
    Talk to Agent shows "Invite an agent to the room first." and starts nothing; if Nova leaves/room
    ends/voice drops, the worker stops and state resets. `novaInRoom` is threaded from
    `room/[roomId]/page.tsx` → `RoomVoice` → `VoiceSession`. Lives in
...
- **Security model = RLS, but policies are not in the repo.** Highest-priority concern.
- **No rate limiting** on `/api/agents/nova/respond` — each call burns Deepgram + Gemini
  + TTS + a LiveKit connection.
- **Wake word:** optional **browser-side** wake word exists for Path A
...
- **Before every significant code change**, keep docs in sync:
  ...
  - update `AGENTS.md` and `CLAUDE.md` if architecture changes,
  - keep env docs synchronized across `.env.example`, `.env.local`, and the READMEs.
- Do not introduce backwards-compat shims or speculative abstractions; do not create docs
  unless asked.
