-- Synq Agent Registration Schema
...
-- do not appear in Synq rooms or the Agent Selector until a later DB-backed
...
begin
  if not exists (
    ...
    where schemaname = 'public'
      ...
      and policyname = 'Synq agents insert own draft or pending review'
  ) then
    -- Rename the historical policy only when the canonical name is absent.
    if exists (select 1 from pg_policies where schemaname='public' and tablename='agents' and policyname='Voxa agents insert own draft or pending review') then
      alter policy "Voxa agents insert own draft or pending review" on public.agents rename to "Synq agents insert own draft or pending review";
    else
    create policy "Synq agents insert own draft or pending review"
...
begin
  if not exists (
    ...
    where schemaname = 'public'
      and tablename = 'agents'
      and policyname = 'Synq agents select own or approved visible'
  ) then
    -- Rename the historical policy only when the canonical name is absent.
    if exists (select 1 from pg_policies where schemaname='public' and tablename='agents' and policyname='Voxa agents select own or approved visible') then
      alter policy "Voxa agents select own or approved visible" on public.agents rename to "Synq agents select own or approved visible";
    else
    create policy "Synq agents select own or approved visible"
...
begin
  if not exists (
    ...
    where schemaname = 'public'
      and tablename = 'agents'
      and policyname = 'Synq agents update own editable records'
  ) then
    -- Rename the historical policy only when the canonical name is absent.
    if exists (select 1 from pg_policies where schemaname='public' and tablename='agents' and policyname='Voxa agents update own editable records') then
      alter policy "Voxa agents update own editable records" on public.agents rename to "Synq agents update own editable records";
    else
    create policy "Synq agents update own editable records"
