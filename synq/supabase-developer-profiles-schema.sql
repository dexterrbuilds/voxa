-- Synq Developer Profiles Schema
...
begin
  if not exists (
    ...
    where schemaname = 'public'
      ...
      and policyname = 'Synq profiles select own profile'
  ) then
    -- Rename the historical policy only when the canonical name is absent.
    if exists (select 1 from pg_policies where schemaname='public' and tablename='developer_profiles' and policyname='Voxa profiles select own profile') then
      alter policy "Voxa profiles select own profile" on public.developer_profiles rename to "Synq profiles select own profile";
    else
    create policy "Synq profiles select own profile"
  ...
  if not exists (
    ...
    where schemaname = 'public'
      and tablename = 'developer_profiles'
      and policyname = 'Synq profiles insert own profile'
  ) then
    -- Rename the historical policy only when the canonical name is absent.
    if exists (select 1 from pg_policies where schemaname='public' and tablename='developer_profiles' and policyname='Voxa profiles insert own profile') then
      alter policy "Voxa profiles insert own profile" on public.developer_profiles rename to "Synq profiles insert own profile";
    else
    create policy "Synq profiles insert own profile"
  ...
  if not exists (
    ...
    where schemaname = 'public'
      and tablename = 'developer_profiles'
      and policyname = 'Synq profiles update own profile'
  ) then
    -- Rename the historical policy only when the canonical name is absent.
    if exists (select 1 from pg_policies where schemaname='public' and tablename='developer_profiles' and policyname='Voxa profiles update own profile') then
      alter policy "Voxa profiles update own profile" on public.developer_profiles rename to "Synq profiles update own profile";
    else
    create policy "Synq profiles update own profile"
