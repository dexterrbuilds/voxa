...
const {
  ...
  parseTools,
} = require("../app/lib/server/agents/runtime/synqMessageClient.ts");
...
test("endpoint policy rejects private networks, credentials and alternate IP spellings", () => {
  ...
  assert.equal(
    deriveMessageEndpoint("https://example.com/synq/handshake/?v=1"),
    "https://example.com/synq/message?v=1",
  );
});
...
test("DNS validation rejects mixed/private answers and pins a public socket address", async (t) => {
  ...
  const input = {
    url: "https://example.com/synq/message",
    body: {},
  ...
  t.mock.method(https, "request", (url, options, done) => {
    ...
    assert.equal(options.headers["X-Synq-Request-Id"], "legacy-request-123");
    assert.equal(options.headers["X-Voxa-Request-Id"], undefined);
...
test("transport reports success, malformed replies, HTTP failures, timeout and cancellation without leaking content", async (t) => {
  ...
  const input = {
    endpointUrl: "https://example.com/synq/handshake",
  ...
  assert.equal((await postSynqMessage(input)).ok, true);
  handler = async () => ({ status: 200, payload: { text: "" } });
  assert.equal((await postSynqMessage(input)).code, "agent_bad_response");
  ...
  assert.equal((await postSynqMessage(input)).code, "agent_timeout");
  const controller = new AbortController();
  controller.abort();
  assert.equal(
    (await postSynqMessage({ ...input, signal: controller.signal })).code,
    "agent_cancelled",
  );
