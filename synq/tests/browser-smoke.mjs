...
const artifacts = process.env.SMOKE_ARTIFACTS || "/private/tmp/synq-smoke";
...
try {
  ...
  const endpoint = page.getByPlaceholder("https://your-agent.example/synq/handshake");
  await endpoint.fill("https://example.com/synq/handshake");
  ...
  await page.evaluate(() => {
    localStorage.setItem("synq-theme", "dark");
  ...
  await page.evaluate(() => {
    localStorage.setItem("synq-theme", "light");
