...
test("legacy Nova room voice, providers, capture and memory remain byte-identical to checkpoint", () => {
  ...
  for (const path of [
    ...
    // Only the reviewed internal fallback brand changed; all other voice bytes must match.
    const expected =
      path === "app/api/agents/nova/respond/route.ts"
        ? baseline.replace('?? "voxa"', '?? "synq"')
        : baseline;
    assert.equal(readFileSync(resolve(root, "synq", path), "utf8"), expected, path);
  }
});
