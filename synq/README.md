...
The Next.js project now lives in `synq/`. Update future deployment roots accordingly. Shared theme
tokens in `app/synq-theme.css` power the marketing app too. Light is the default;
`synq-theme` is canonical. Theme, dormant auth and room state migrate once from old keys.
Provider credential names are unchanged. Public deployment URLs are explicitly configured.
...
Use `X-Synq-Request-Id` and the canonical SDK; deprecated request headers/wire clients remain
supported at the compatibility boundary. See [naming migration](../docs/synq-naming-migration.md).
...
4. Add these redirect URLs in Authentication -> URL Configuration -> Redirect URLs:
   ...
   - your deployed app login URL, for example `https://your-staging-app.example/login?verified=true`
...
- **Adapter contract:** an imported agent wraps its runtime behind the **same** Synq endpoints
  (`GET /health`, `POST /synq/handshake`, `POST /synq/message`, optional `POST /synq/voice`).
...
`{ type: "synq.handshake" }` to the agent's `endpoint_url` (5s timeout) and runs three checks:

1. `endpoint_reachable` — the endpoint answers the handshake,
2. `sdk_compatible` — protocol is `synq-agent` and the SDK version is supported,
3. `capability_payload` — declared capabilities are covered by what the endpoint reports.

The handshake contract mirrors `@synq/sdk`'s `createAgentHandshake()`. Admins run it via
...
and re-checked against the DB every call), then the `SandboxRuntime` POSTs the SDK `synq.message`
contract (`{ type: "synq.message", message, context: { sandbox: true } }`) to each agent's message
endpoint (derived from the registered handshake URL: `…/synq/handshake` → `…/synq/message`) and
...
- **Cleanup** — on room close (`leaveSharedRoom`) the SECURITY DEFINER fn
  ...
  room (best-effort; the existing hourly room-cleanup sweep is the backstop), preserving
...
share one wire client (`app/lib/server/agents/runtime/synqMessageClient.ts`); a future
...
User taps "Talk to {agent}" → silence-bounded clip (its OWN mic stream)
  ...
  → VoiceAgentRuntime POSTs { type:"synq.voice", message:<transcript>, context:{mode:"voice_beta",roomId,agentId,history} }
...
notices — are preserved). The existing hourly room-cleanup sweep removes all
