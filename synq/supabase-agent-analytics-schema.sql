...
begin
  if not exists (
    ...
    where schemaname = 'public'
      ...
      and policyname = 'Synq analytics select own agents'
  ) then
    -- Rename the historical policy only when the canonical name is absent.
    if exists (select 1 from pg_policies where schemaname='public' and tablename='agent_analytics' and policyname='Voxa analytics select own agents') then
      alter policy "Voxa analytics select own agents" on public.agent_analytics rename to "Synq analytics select own agents";
    else
    create policy "Synq analytics select own agents"
