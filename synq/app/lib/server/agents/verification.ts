...
import { agentWireType, isSupportedAgentProtocol } from "@/lib/agents/protocol-compatibility";
...
// developer SANDBOX only. The handshake contract below mirrors `@synq/sdk`'s
...
export async function fetchHandshake(
  ...
  try {
    const response = await requestAgentJson({
      url: endpointUrl,
      body: { type: agentWireType(endpointUrl, "synq.handshake") },
...
export async function runAgentVerification(agent: VerifiableAgent): Promise<VerificationReport> {
  ...
  const protocolOk = isSupportedAgentProtocol(handshake.protocol);
  const versionOk =
  ...
  checks.push({
    ...
    detail:
      protocolOk && versionOk
        ...
        : !protocolOk
          ? `Unexpected protocol "${handshake.protocol ?? "none"}" (expected "${SYNQ_AGENT_PROTOCOL}").`
