...
import { agentWireType } from "@/lib/agents/protocol-compatibility";

// Shared transport for the `synq.message` wire contract. Both SandboxRuntime and
...
export async function postSynqMessage(input: {
  signal?: AbortSignal;
  requestId?: string;
  endpointUrl: string;
  message: string;
  ...
  timeoutMs?: number;
  // Wire `type` to send. Defaults to "synq.message"; VoiceAgentRuntime passes
  // "synq.voice". The reply shape is the same (`{ text }` required).
  type?: string;
  ...
  try {
    ...
    const response = await requestAgentJson({
      ...
      requestId,
      body: {
        type: agentWireType(input.endpointUrl, input.type ?? SYNQ_MESSAGE_TYPE),
