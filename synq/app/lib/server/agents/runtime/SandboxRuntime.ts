...
import {
  ...
  postSynqMessage,
  SYNQ_MESSAGE_TYPE,
} from "@/lib/server/agents/runtime/synqMessageClient";

// SandboxRuntime: the developer-sandbox agent transport.
//
// It sends a `synq.message` request to a verified agent's message endpoint and
...
//
// Shares its wire transport with RoomTextRuntime via `synqMessageClient`. A future
...
export { deriveMessageEndpoint, SYNQ_MESSAGE_TYPE };
