...
const roomStorageKey = "synq-room-storage";
...
export function authDebug(event: string, data?: Record<string, unknown>) {
  if (AUTH_DEBUG) {
    console.info(`[synq-auth] ${event}`, data ?? {});
  }
}
...
function readSessionValue<T>(key: string): T | null {
  ...
  try {
    const value = readMigratedStorage(window.sessionStorage, key);
...
function reportMissingSupabase(message: string) {
  console.error(
    "Supabase Auth is not configured. Set NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY in synq/.env.local.",
  );
  return message;
}
...
function normalizeRoom(room: Partial<Room> & { host?: string }): Room {
  ...
  return {
    ...
    createdBy: room.createdBy ?? room.host ?? "synq",
